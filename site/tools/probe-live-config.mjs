...
// copy is the template new Cloudflare projects start from; this copy is what actually
// gates disabilitywiki.org. They are two files, which is the very duplication this probe
// exists to police — so when you change one, re-copy, and re-run
// ~/.claude/skills/wrangler/tests/test_probe_live_config.sh against both.
